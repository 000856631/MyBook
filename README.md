...
后续注意。
2、推荐图书几个功能。练练爬虫技术.本地基于命令行的抓取工程搭建了一个，简单，用来练习cheerio框架还是不错的。
推荐图书的功能完成了。
3、基于现有的表数据书友统计。
...
把爬虫框架也搭建起来吧
爬取思路如下：
首页面爬取到的列表，记入数据库 首页列表页。 表字段 
生产上新设立一张表，弄一个链接触发这个查询，把数据库记录插入进去。
然后再按照数据库的记录爬取，逐条记录到详情页，归属到特定的用户（公用用户下面)
最后查询的前端界面就按照送入特定的公用用户id来查询。
另外一种思路，直接查完二级就算，这种思路简单点，无需建表了。
先弄好入口，然后做好打印。

show create table 可以展示DDL的建表语句
测试环境已经能爬取成功了。
我在本地搞了一套爬虫本地运行环境，调试成功后部署到测试环境已经成功，最终还是把生产环境也调通吧。
下来的思路是测试环境把数据爬进数据库，然后再手工迁移到生产环境，先这么弄吧。
再开发一个小程序从新数据库取数的过程。
云开发貌似可以放很多文档,限制比较多，下来可以研究下。
Cool，测试环境已经调通，切换生产记得更改数据库配置的密码
生产也请求完了，好多之前的旧代码也没有删除。
然而却发现一不小心把生产的数据库都给删了。。。Delete的操作要谨慎啊，它不像Oracle 需要多commit一下才生效。。。
深深的感觉对不起我曾经的6W用户，没办法，个人的数据库就是这么随意，一不小心就删掉了。
转头一想，辛苦折腾了几天居然就是在数据库插入几条，感觉还是有点挫。
另外考虑下新建一张图书表吧，可以作为自己的书籍资料资产，后续可以封装一下API接口出来。
这个就叫做燕过留痕
提交了小程序审核，等待审核ing
设计一套合理的表结构，把过程的sql存储起来，可以通过js代码执行。
以前的代码逻辑迁移到新表结构处理。

2019-02-11 中午没午睡打了几局王者荣耀，又到了混搭的时期，感觉整个人有点浑浑噩噩。
什么都不想干，什么都不想动，就想着发呆，顺便期待着我的小米8，整个人感觉很丧很颓废。
表设计出来了，也不想继续编下去。就如同当年念书，不想继续看书不想学习动感觉。
为啥会有这种感觉？感觉是多巴胺分泌不足？
现在连小米8也不期待了，大概是发现送了雅典娜也玩不赢的那种感觉吧。雅典娜的三连我就成功一次。
提高成功率的方法，2跳过去，A，然后顶对面，2再跳一下拉扯一下，等几秒，a过去，然后再顶一下，2A结束。
移速鞋来快速卡刀切2技能会不会好一点？ 一个2技能，算好时间一个2技能过去，然后对a几下，2技能好了再来一次，然后秒接12完成。
可以试试。物理打野刀，移速鞋，吸血刀，火甲，反甲，破军。 带线。靠速度，靠灵活。
...
2019-02-26
接到了同学一个外包小请求。
顺手接触来一些thinkJs和eggjs,thinkjs有现成的电商和小程序模版，拿过来稍微改改就能适用于大部分的
外包场景
egg.js对koa框架做了一层规范，适用于企业级的nodejs框架，属于内核级别的框架。
要不要涉足外包这个行业呢？ 可以稍微赚取一些外快，利用第三方外包公司多人力的缺陷，优势上当前互联网node这块
开源其实挺多的，拿一些现成的东西稍微改改，就能适用很多场景，支持一些小公司开始实现信息化。
至于为啥要用node,主要还是因为我免费的服务器底层只能用node。
比较尴尬上当前职业主业还是java框架的。
研究这些做外包，是否会对我主业造成一些影响？是好还是坏？
做这些是否能对我算是有益的项目经验？
在工行更上层楼，无疑是去往业务上靠拢，熟悉业务规则，熟悉业务人员，熟悉金融形态。
技术上也是偏向金融业务方向。
另外一个方向就是搞外包弄些外快，最终会不会如同达哥一样，为了短期的利益浪费核心技能，只能成为一个杂耍程序员？
能快速创造一个成品感觉确实很不错，而且还有额外的收入，不过毕竟不是核心技术。
我应该把时间、精力用于打造自己的护城河上，而java方向，就是最深的护城河，这么多年了依旧是企业级最核心的技术。
Java才是成为一个IT架构师的金钥匙。
现在，我的当务之急是补全自己java系的技术短板，成长为一名合格的具备分布式容器能力的金融架构师，而不是和
一些外包公司红海去竞争打滚，IT人应该有所为有所不为。
发掘工行的核心竞争力，然后去发扬光大，小微的外包需求，其实并不能带来核心竞争力的提升，只适合业余时间用来娱乐，
而不是用来实现个人价值的提升。

今天才发现一个巨坑爹的事情，SSL的证书需要我自己手工申请，然后通过工单的方式补充给客服去替换，真是蛋疼啊。

然后又发现一个巨坑爹的事情，豆瓣居然把查询ISBN号的API给封了，不过没关系，对于会爬虫技术的人来说，一切网页皆为API.动手开干。
页面地址：
https://book.douban.com/subject_search?search_text=9787020054985&cat=1001
拿到
<a href="https://book.douban.com/subject/1988083/" data-moreurl="" class="cover-link"><img src="https://img1.doubanio.com/view/subject/l/public/s5813487.jpg" alt="明堂制度源流考" class="cover"></a>
获得二级跳转链接，然后进入最终解析页，像以前已有解析即可
https://book.douban.com/subject/1988083/
后续也把书籍信息插入自己的数据库吧。
写个定时批量，定期去爬取更新年度推荐的书籍目录。
